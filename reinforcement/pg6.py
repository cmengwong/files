...
# this is the extend of the pg4.py (using TD error)
# I have to predict the right action of grid 500*500
# by training by grid 200*200
# Therefore, what i gonna do in the progamme is that
# I have to train the reinforcement learning of grid 200*200
# After that, use the last training data to run a 500*500 grid.

# or find a way to save the data
...
def play_one_td(laplace_env, pmodel, vmodel, gamma, last = False):
	...
	while not done and iters < 200:
		...
		observation, reward, done, info = laplace_env.solve_step(r = ratio)
...
def last_play(laplace_env, pmodel, vmodel):
	error = laplace_env.reset_grid()
	done = False
	iters = 0
	observation = laplace_env.from_error_to_input_of_NN(error, r = 0.1)
	...
	while not done:
		action = pmodel.sample_action(observation)
		ratio = (action + 1) / 10
		observation, _, done, err = laplace_env.solve_step(r = ratio)
		errors.append(err)
		iters += 1


	return iters*1000, 	errors

def laplace_env_main():
	session = tf.InteractiveSession()
	# train by a 200*200 grid
	laplace_env = create_LaplaceSolver(session, n = 300)
	D = laplace_env.trained_NN.D
	K = 10
	pmodel = PolicyModel(D, K, [20,10])
	vmodel = ValueModel(D, [20,10])
	...
	for n in range(N):
		if (n == N):
			ite, all_error = play_one_td(laplace_env, pmodel, vmodel, gamma)
		else:
			ite, _ = play_one_td(laplace_env, pmodel, vmodel, gamma)
