...
######################################
# this is the function that use pg_random
# and output a discrete actions.
#
######################################
...
class PolicyModel:
	def __init__(self, D, hidden_layer_sizes= []):
		...
		###final layer###
		layer = HiddenLayer(M1, 10, lambda x:x, use_bias = False, zeros =True)
		self.layers.append(layer)
	...
	def params_reshape(self, shapes, params):     # reshape to be a matrix
		...
		for i, shape in enumerate(shapes):  # flat params to matrix
			...
			self.layers[i].params = [params[start: start + n_w].reshape(shape),
